...
$(function() {
	...
	$("#btnAdmin").click(function() {
		location.href="${path}/swup_member/list.do";
	});
			...
			<div class="bg-3">
				<form action="${path}/swup_post/search.do" name="form1" method="post">
					...
					<div style="float: right" class="margin">
						<select name="array">
							<c:choose>
								<c:when test="${array=='num desc'}">
									<option value="num desc" selected>최신순</option>
									<option value="num">날짜순</option>
									<option value="readcount desc">조회순</option>
									<option value="recommendcount desc">추천순</option>
								</c:when>
								<c:when test="${array=='num'}">
									<option value="num desc">최신순</option>
									<option value="num" selected>날짜순</option>
									<option value="readcount desc">조회순</option>
									<option value="recommendcount desc">추천순</option>
								</c:when>
								<c:when test="${array=='readcount desc'}">
									<option value="num desc">최신순</option>
									<option value="num">날짜순</option>
									<option value="readcount desc" selected>조회순</option>
									<option value="recommendcount desc">추천순</option>
								</c:when>
								<c:when test="${array=='recommendcount desc'}">
									<option value="num desc">최신순</option>
									<option value="num">날짜순</option>
									<option value="readcount desc">조회순</option>
									<option value="recommendcount desc" selected>추천순</option>
								</c:when>
							</c:choose>
						</select>
					...
					<table align="center" style="width: 100%">
						...
						<tr align="center">
							<td colspan="6" style="border-bottom: 2px solid gray;">
								<c:if test="${page1.curPage > 1 }">
									<a href="#" onclick="list('1')">[처음]</a>
								</c:if>
								<c:if test="${page1.curBlock > 1 }">
									<a href="#" onclick="list('${page1.prevPage}')">[이전]</a>
								</c:if>
								<c:forEach var="num" begin="${page1.blockStart}" end="${page1.blockEnd}">
									<c:choose>
										<c:when test="${num == page1.curPage}">
								...
								<c:if test="${page1.curBlock < page1.totBlock}">
									<a href="#" onclick="list('${page1.nextPage}')">[다음]</a>
								</c:if>
								<c:if test="${page1.curPage < page1.totPage}">
									<a href="#" onclick="list('${page1.totPage}')">[끝]</a>
								</c:if>
							</td>
						</tr>
					</table>
